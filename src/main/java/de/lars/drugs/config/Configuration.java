    ...
    private static class WriteRunner implements Runnable {
        ...
        public void run() {
            synchronized (this.configFile) {
                ...
                try {
                    // Erstelle die Elternverzeichnisse, falls sie nicht existieren
                    File parentDir = this.configFile.getParentFile();
                    if (!parentDir.exists() && !parentDir.mkdirs()) {
                        Configuration.LOGGER.log(Level.SEVERE, "Failed to create config directories: " + parentDir);
                        return;
                    }
                    if (!this.configFile.exists()) {
                        Configuration.LOGGER.log(Level.INFO, "Creating Empty config file: " + this.configFile);
                        if (!this.configFile.createNewFile()) {
                            Configuration.LOGGER.log(Level.SEVERE, "Failed to create config file: " + this.configFile);
                            return;
                        }
                    }
