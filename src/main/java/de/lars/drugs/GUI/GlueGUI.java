...
public class GlueGUI {
    ...
    public GlueGUI(Drugs plugin, Configuration config) {
        this.config = config;
        this.plugin = plugin;
    }

    public void open(Player player) {
        ChestGui gui = new ChestGui(5, "Glue Crafting Recipe");
        ...
        ItemStack resultitem = createGlue(plugin, config);
        ...
        for (char c = 'A'; c <= 'I'; c++) {
            String materialKey = "glue_material_" + Character.toLowerCase(c);
