...
public class ZippoGUI {
    private final Configuration config;
    private final Drugs plugin;

    public ZippoGUI(Drugs plugin, Configuration config) {
        this.config = config;
        this.plugin = plugin;
    }

    public void open(Player player) {
        ChestGui gui = new ChestGui(5, "Zippo Crafting Recipe");
        ...
        ItemStack resultitem = createZippo(plugin, config);
        ...
        for (char c = 'A'; c <= 'I'; c++) {
            String materialKey = "zippo_material_" + Character.toLowerCase(c);
