...
public class SyringeGUI {
    private final Configuration config;
    private final Drugs plugin;

    public SyringeGUI(Drugs plugin, Configuration config) {
        this.config = config;
        this.plugin = plugin;
    }

    public void open(Player player) {
        ChestGui gui = new ChestGui(5, "Syringe Crafting Recipe");
        ...
        // Result Item (Syringe)
        ItemStack resultitem = createSyringe(plugin, config);
        ...
        for (char c = 'A'; c <= 'I'; c++) {
            String materialKey = "syringe_material_" + Character.toLowerCase(c);
