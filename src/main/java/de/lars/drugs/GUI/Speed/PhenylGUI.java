...
public class PhenylGUI {
    private final Configuration config;
    private final Drugs plugin;

    public PhenylGUI(Drugs plugin, Configuration config) {
        this.config = config;
        this.plugin = plugin;
    }

    public void open(Player player) {
        ChestGui gui = new ChestGui(5, "Phenyl Crafting Recipe");
        ...
        ItemStack resultitem = createPhenyl(plugin, config);
        ...
        ItemStack roseBushItem = new ItemStack(Material.ROSE_BUSH);
        GuiItem roseBushGuiItem = new GuiItem(roseBushItem, event -> {
            event.getWhoClicked().sendMessage("You can't take it!");
            event.setCancelled(true);
        });

        pane.addItem(sugarCaneGuiItem, 1, 1);
        pane.addItem(roseBushGuiItem, 2, 2);
