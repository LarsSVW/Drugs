package de.lars.drugs.GUI.Speed;
...
public class SpeedGUI {
    ...
    public SpeedGUI(Drugs plugin, Configuration config) {
        this.config = config;
        this.plugin = plugin;
    }

    public void open(Player player) {
        ChestGui gui = new ChestGui(5, "Speed Crafting Recipe");
        ...
        ItemStack resultitem = createSpeed(plugin, config);
        GuiItem resultItem = new GuiItem(resultitem, event -> {
            event.getWhoClicked().sendMessage("You can't take it!");
            event.setCancelled(true);
        });

        ItemStack phenylItem = new ItemStack(createPhenyl(plugin, config));
        GuiItem phenylGuiItem = new GuiItem(phenylItem, event -> {
            event.getWhoClicked().sendMessage("You can't take it!");
            event.setCancelled(true);
        });
        ...
        pane.addItem(phenylGuiItem, 3, 1);
        pane.addItem(natriumGuiItem, 2, 2);
        pane.addItem(phenylGuiItem, 1, 3);
        pane.addItem(hydrogenGuiItem, 3, 3);
